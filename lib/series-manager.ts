...
export class SeriesManager {
  ...
  async createSeries(options: SeriesCreationOptions): Promise<SeriesFacts> {
    try {
      ...
      await claudeCache.cacheHierarchicalFacts(
        ...
        seriesFacts,
        'series',
        { ttl: 86400 * 30 } // 30 days for series-level facts
      )
  ...
  async trackCrossBookContinuity(bookId: string, characterArcs: any[]): Promise<ContinuityAnalysis> {
    try {
  ...
  private async analyzeSeriesContinuity(
    ...
    try {
      ...
      return analysis as ContinuityAnalysis
    } catch (error) {
  ...
  private async analyzeOverallSeriesContinuity(seriesId: string, books: any[]): Promise<ContinuityAnalysis> {
    try {
      // Get series facts
      const seriesFacts = await this.getSeriesFacts(seriesId)
      ...
      return overallAnalysis as ContinuityAnalysis
    } catch (error) {
  ...
  private buildSeriesInitializationPrompt(options: SeriesCreationOptions & { seriesId: string }, universe: any): string {
    return `
...
**Output Format:** Valid JSON matching the SeriesFacts interface.
  ...
  private buildContinuityAnalysisPrompt(seriesFacts: SeriesFacts, previousBooks: any[], characterArcs: any[]): string {
    return `
...
**Format:** Structured markdown document suitable for reference by writers and editors.
  ...
  private async getSeriesFacts(seriesId: string): Promise<SeriesFacts | null> {
    try {
      const cached = await claudeCache.getOptimizedFactContext(seriesId, 'series')
      if (cached) {
        return cached as SeriesFacts
      }
  ...
  private async getUniverseData(universeId: string): Promise<any> {
    // Implementation for getting universe data
    return null
  }

  private async getPreviousBooks(seriesId: string, currentBookId: string): Promise<any[]> {
  ...
  private async storeSeriesBible(seriesId: string, bible: any): Promise<void> {
  ...
  private async validateCharacterUpdate(seriesId: string, characterName: string, character: SeriesCharacter): Promise<any> {
