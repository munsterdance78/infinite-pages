...
export class ClaudeBatchProcessor {
  ...
  private async processNext(): Promise<void> {
    ...
    try {
      ...
      if (this.retryFailed && (operation as any).retries < this.maxRetries) {
        (operation as any).retries = ((operation as any).retries || 0) + 1
        this.queue.push(operation) // Add back to queue for retry
      }
    } finally {
