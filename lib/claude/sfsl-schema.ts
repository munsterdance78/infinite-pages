...
export class SFSLProcessor {
  compressFacts(extractedFacts: any): string {
    const compressed = extractedFacts.map((fact: any) => {
      return `${fact.type}:${fact.content.slice(0, 50)}...`
    })
    return compressed.join('|')
  }

  expandForDisplay(sfslData: string): any {
    try {
      ...
      return { facts: [], metadata: { error: 'Invalid SFSL data' } }
    }
  }
