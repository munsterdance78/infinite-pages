...
class RequestTracker {
  ...
  async trackApiCall<T>(
    ...
    try {
      ...
      const requestLog: RequestLog = {
        ...
        apiEndpoint: url,
        expectedEndpoint: options.expectedEndpoint || context.expectedEndpoint,
        httpMethod: method,
        requestHeaders,
        requestBodySize,
        ...
        successFlag,
        errorMessage: successFlag ? undefined : `HTTP ${response.status}: ${response.statusText}`,
        errorCategory: this.categorizeError(response.status),
        integrationPoint: context.integrationPoint,
        expectedIntegration: context.expectedIntegration,
        integrationSuccess,
  ...
  completeRequest(requestId: string, uiUpdateSuccess: boolean = true, customData?: Record<string, any>) {
    const requestData = this.activeRequests.get(requestId)
    if (!requestData) {
...
export async function trackRequest<T>(
  ...
  return requestTracker.trackCompleteFlow(
    context,
    async (requestId) => {
      // Modify the fetch options to include request ID
      const originalFetch = window.fetch
      window.fetch = (input, init = {}) => {
        return originalFetch(input, { ...init, requestId })
      }
      ...
      try {
        ...
        // (This is handled by the interceptor, but good to be explicit)
      }
    }
  )
}
