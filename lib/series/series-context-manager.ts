...
export class SeriesContextManager {
  ...
  async getOptimizedSeriesContext(
    ...
    return {
      series_core: {
        ...
        central_series_conflict: this.extractCentralConflict(series),
  ...
  private buildCurrentWorldState(series: any, worldChanges: any[]): WorldState {
    ...
    return {
      ...
      changed_since_book_one: latestChanges
    }
  }

  private extractUnresolvedConflicts(plotThreads: any[]): UnresolvedConflict[] {
    return plotThreads
  ...
  private filterActivePlotThreads(plotThreads: any[], currentBook: number): PlotThread[] {
  ...
  private extractMainPlot(chapters: any[]): string {
    // Extract main plot from chapter summaries and content
    return chapters.map(ch => ch.summary).join(' ').slice(0, 200)
  }

  private extractSubplots(chapters: any[]): string[] {
    // Extract subplot threads from chapters
    return []
  }

  private extractCharacterDevelopments(chapters: any[]): string[] {
    // Extract character development from chapters
    return []
  }

  private extractWorldChanges(chapters: any[]): string[] {
    // Extract world changes from chapters
    return []
  }

  private extractUnresolvedElements(chapters: any[]): string[] {
    // Extract unresolved elements from chapters
    return []
  }

  private extractCliffhangers(chapters: any[]): string[] {
    // Extract cliffhangers from chapters
    return []
  }

  private extractNewCharacters(chapters: any[]): string[] {
    // Extract new character introductions
    return []
  }

  private extractDeadCharacters(chapters: any[]): string[] {
    // Extract character deaths
    return []
  }

  private selectActiveCharacters(arcs: CharacterArc[], chapterPlan: any, maxCount: number): any[] {
    // Select most relevant characters for this chapter
    return arcs.slice(0, maxCount)
  }
  ...
  private determineWorldType(worldRules: any): string {
    // Determine world type from rules
    return worldRules?.geography?.world_type || 'earth_like'
  }

  private extractCentralConflict(series: any): string {
  ...
  private extractRelevantRelationships(characters: any[]): Record<string, string> {
  ...
  private getImportantRules(worldRules: any): string[] {
  ...
  private extractPoliticalSituation(changes: any[]): string { return 'stable' }
  private extractTechnologyLevel(worldRules: any): string { return 'modern' }
  private extractSocialClimate(changes: any[]): string { return 'normal' }
  private extractCurrentThreats(changes: any[]): string[] { return [] }
  private extractCurrentOpportunities(changes: any[]): string[] { return [] }
  private determineConflictType(description: string): any { return 'personal' }
  private extractStakes(description: string): string { return 'unknown stakes' }
  private calculateEscalationLevel(thread: any): number { return 5 }
  private assessTensionLevel(thread: any): any { return 'moderate' }
}
