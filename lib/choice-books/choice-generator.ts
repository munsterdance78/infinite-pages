...
Return as JSON:
{
  ...
  private parseAndValidateChoices(generatedContent: string, context: ChoiceOptimizedContext) {
    try {
      ...
      const validatedChoices = parsed.choices.map((choice: any, index: number) => ({
  ...
  private analyzeConsequenceDepth(choices: ChoiceMade[], context: any): 'immediate' | 'short_term' | 'long_term' | 'ending_affecting' {
    if (context.ending_proximity > 0.8) return 'ending_affecting'
  ...
  private getAvailablePaths(context: any, choices: ChoiceMade[]): string[] {
  ...
  private calculateEndingProximity(context: any, choices: ChoiceMade[]): number {
  ...
  private assessChoiceDifficulty(choice: any, context: ChoiceOptimizedContext): 'easy' | 'moderate' | 'hard' {
  ...
  private determineChoiceType(choices: any[]): 'binary' | 'multiple' | 'consequential' {
    if (choices.length === 2) return 'binary'
    if (choices.some(c => c.consequences?.length > 1)) return 'consequential'
    return 'multiple'
  }
