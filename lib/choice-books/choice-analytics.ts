...
export class ChoiceBookAnalytics {
  ...
  private analyzeReaderPaths(paths: any[]): PathAnalysis {
    if (paths.length === 0) {
      return {
        total_paths: 0,
  ...
  private analyzeReaderBehavior(paths: any[]) {
  ...
  private groupSimilarPaths(paths: any[]) {
  ...
  private extractChoicePattern(choices: any[]): string {
